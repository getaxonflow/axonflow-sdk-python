...
Real-wire proof, through the Python SDK's own runtime against a **live
enterprise stack**, that `explain_decision` and `list_decisions` are scoped to
the identity the caller presents - and that the SDK reports the outcomes
honestly instead of collapsing them into "nothing there".
...
"a caller who sees everything" - it is a caller the platform cannot scope.
Measured live on this stack before the fix:
...
| 4 | List with **no identity** | Must be a typed `ReadScopeError` with `identity_missing`, never `[]`. A stack that returns rows here fails loudly - every other scoping assertion would be vacuous |
| 5 | Explain dev-a's decision **as dev-b** | Must refuse, and must **not** report a missing identity - dev-b presented one |
| 6 | **Malformed / expired / another-org** tokens | Each must fail **closed** with 401 and must not echo the credential |
...
fire a genuine ping at the production checkpoint. Redirecting `HOME` does not
work - the path is resolved when the heartbeat state is constructed.
